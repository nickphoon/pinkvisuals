...
function checkInputErrors(input, type, endsAtLastNode = false) {
    ...
    if (isNaN(input)) {
        error.innerHTML = type + " must be a number";
        inputError = true;
    }
    else if (type === "Index" && (input > end || input < 0)) {
        error.innerHTML = "Index Out Of Bounds";
        inputError = true;
    }
    if (inputError) 
		error.firstChild.style.animation = "highlightNode .8s ease"; 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('remove-btn').addEventListener('click', function() {
        let userInput = getUserInput(this.parentNode);
	if (!isNaN(userInput.index) && !isNaN(userInput.data)) {
	    error.innerHTML = "Remove either by index or data, not both";
	    return;
	}
	else if (!isNaN(userInput.index)) {
            removeIndex(userInput.index);
        } else {
            removeData(userInput.data);
 	}
    }); 
});